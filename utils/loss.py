    ...
    for i in range(3):
        for j in range(3):
            for k in range(3):
                ...
                target = (base + torch.LongTensor([i, j, k], device=x.device)).clamp(0, size - 1)
...
def soft_iou(x, y, size=16, pkg='numpy', dense=True, soft=False, normalize=False, visualize=False):
    if pkg == 'numpy':
...
def iou(x, y, voxel_size=0.01, pkg='numpy', visualize=False):
    ...
    if pkg == 'numpy':
        ...
        grid_x = torch.zeros([int((max_bound[0] - min_bound[0]) / voxel_size) + 1,
                            int((max_bound[1] - min_bound[1]) / voxel_size) + 1,
                            int((max_bound[2] - min_bound[2]) / voxel_size) + 1])
