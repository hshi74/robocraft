...
def plot_train_loss(loss_dict, path=''):
    ...
    for label, loss in loss_dict.items():
        if not 'loss' in label and not 'accuracy' in label: continue
...
def plot_eval_loss(title, loss_dict, loss_std_dict=None, alpha_fill=0.3, colors=None, 
    ...
    for label, loss in loss_dict.items():
        ...
        if loss_std_dict:
            ...
            plt.fill_between(time_list, loss_max_bound, loss_min_bound, color=colors[i % len(colors)], alpha=alpha_fill)
...
def visualize_points(ax, args, particles, draw_set, target, mask=None, rels=None, 
    axis_off=False, focus=True, res='high'):
    ...
    if 'dough' in draw_set:
        if mask is None:
            ...
            c = mask
        points = ax.scatter(particles[:args.n_particles, args.axes[0]], 
                            particles[:args.n_particles, args.axes[1]], 
                            particles[:args.n_particles, args.axes[2]], c=c, s=point_size)
        outputs.append(points)
    if 'floor' in draw_set:
        floor = ax.scatter(particles[args.n_particles:args.n_particles+args.floor_dim, args.axes[0]], 
                           particles[args.n_particles:args.n_particles+args.floor_dim, args.axes[1]], 
                           particles[args.n_particles:args.n_particles+args.floor_dim, args.axes[2]], c='r', s=point_size)
        outputs.append(floor)
    if 'tool' in draw_set:
        tool = ax.scatter(particles[args.n_particles+args.floor_dim:, args.axes[0]], 
                          particles[args.n_particles+args.floor_dim:, args.axes[1]], 
                          particles[args.n_particles+args.floor_dim:, args.axes[2]], c='r', s=point_size, zorder=4.2)
        outputs.append(tool)
    ...
    if target is not None:
        ax.scatter(target[:, args.axes[0]], target[:, args.axes[1]], target[:, args.axes[2]], c='c', s=point_size, alpha=0.3)
...
# @profile
def render_anim(args, row_titles, state_seqs, attn_mask_pred=None, rels_pred=None, 
    draw_set=['dough','floor','tool'], axis_off=False, target=None, views=[(90,-90),(0,-90),(45,-45)], 
    fps=15, res='high', path=''):
    ...
    fig, big_axes = plt.subplots(n_rows, 1, figsize=(fig_size * n_cols, fig_size * n_rows))
    ...
    for i in range(n_rows):
        ...
        for j in range(n_cols):
            ...
            outputs = visualize_points(ax, args, state_seqs[i][0], draw_set, target_cur, 
                mask=mask, rels=rels, res=res, axis_off=axis_off)
    ...
    def update(step):
        ...
        for i in range(n_rows):
            ...
            for j in range(n_cols):
                ...
                if 'floor' in draw_set:
                    outputs[draw_idx]._offsets3d = (
                        frame_cur[args.n_particles:args.n_particles+args.floor_dim, args.axes[0]], 
                        frame_cur[args.n_particles:args.n_particles+args.floor_dim, args.axes[1]], 
                        frame_cur[args.n_particles:args.n_particles+args.floor_dim, args.axes[2]])
                    draw_idx += 1
                if 'tool' in draw_set:
                    outputs[draw_idx]._offsets3d = (
    ...
    anim = animation.FuncAnimation(fig, update, frames=np.arange(0,n_frames+2*fps), blit=True)
...
def render_frames(args, row_titles, state_seq, frame_list=[], axis_off=True, focus=True, 
    ...
    for frame in frame_list:
        for i in range(n_rows):
            ...
            if n_rows == 1:
                big_axes.set_title(row_titles[i], fontweight='semibold', fontsize=title_fontsize)
                big_axes.axis('off')
            else:
                big_axes[i].set_title(row_titles[i], fontweight='semibold', fontsize=title_fontsize)
                big_axes[i].axis('off')

            for j in range(n_cols):
                ax = fig.add_subplot(n_rows, n_cols, i * n_cols + j + 1, projection='3d')
                ax.view_init(*views[j])
                visualize_points(ax, args, state[frame], draw_set, target_cur, axis_off=axis_off, focus=focus_cur, res=res)
            ...
            for k in range(len(geometry_list)):
                ...
                if type == o3d.geometry.Geometry.Type.TriangleMesh:
                    ...
                    if len(label_list) > 0:
                        label = label_list[k]
                        if 'dough' in label:
                            ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c='b', s=point_size, label=label)
                        elif 'tool' in label:
                            ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c='r', alpha=0.2, zorder=4.2, s=point_size, label=label)
                        else:
                            ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c='yellowgreen', zorder=4.1, s=point_size, label=label)
                    else:
                        label = None
                        ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c=colors, s=point_size, label=label)
                else:
...
def visualize_neighbors(args, particles, target, neighbors, path=''):
    ...
    ax.scatter(particles[neighbors, args.axes[0]], particles[neighbors, args.axes[1]], particles[neighbors, args.axes[2]], c='b', s=60)
    ax.scatter(particles[target, args.axes[0]], particles[target, args.axes[1]], particles[target, args.axes[2]], c='r', s=60)
...
def plot_cm(test_set, y_true, y_pred, path=''):
    ...
    cm_display = metrics.ConfusionMatrixDisplay(confusion_matrix=confusion_matrix, display_labels=test_set.classes)
...
def visualize_pcd_pred(row_titles, state_list, views=[(90,-90),(0,-90),(45,-45)], axis_off=False, res='low', path=''):
    ...
    for i in range(n_rows):
        ...
        for j in range(n_cols):
            ...
            ax.scatter(state[:, 0], state[:, 1], state[:, 2], c=state_colors, s=point_size)
