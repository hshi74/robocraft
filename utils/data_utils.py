...
def get_env_group(args, B):
    ...
    scene_params = torch.tensor(args.scene_params, device=args.device, dtype=torch.float32)
...
def load_tool_repr(args):
    ...
    for tool_name, tool_geom_list in args.tool_geom_mapping.items():
        ...
        for i in range(len(tool_geom_list)):
            tool_repr_points_path = os.path.join(args.tool_repr_path, f'{tool_geom_list[i]}_points.npy')
...
def get_act_seq_from_state_seq(args, state_shape_seq):
    ...
    for i in range(1, state_shape_seq.shape[0]):
        ...
        if args.full_repr and 'roller' in args.env:
            roller_motion = np.mean(state_shape_seq[i, tool_start:], axis=0) - \
                np.mean(state_shape_seq[i - 1, tool_start:], axis=0)
            ...
            if spread:
                ...
                if roller_motion_xy_dist > 0:
                    roll_norm = np.cross(roller_motion[:2], (state_shape_seq[i, -1] - state_shape_seq[i, tool_start]))
            ...
            for j in range(len(args.tool_dim[args.env])):
                tool_dim = args.tool_dim[args.env][j]
                state_diff = state_shape_seq[i, tool_start] - state_shape_seq[i - 1, tool_start]
