...
class DynamicsPredictor(nn.Module):
    def __init__(self, args, residual=False):
        ...
        # ParticleEncoder
        particle_input_dim = self.attr_dim + 1 + (self.n_his*self.time_step+1) * self.state_dim + self.mem_dim
        self.particle_encoder = Encoder(particle_input_dim, nf_particle, nf_effect)

        # RelationEncoder
        relation_input_dim = particle_input_dim * 2 + 1 + (self.n_his*self.time_step+1) * self.state_dim
    ...
    def forward(self, inputs):
        ...
        state_res_norm = torch.zeros(B, self.n_his*self.time_step, N, self.state_dim, device=self.device)
        state_res_norm[:, :, n_p + args.floor_dim:, :3] = batch_normalize(action_his[:, 1:, :, :3] - action_his[:, :-1, :, :3], mean_d, std_d)
        ...
        state_norm_t = state_norm.transpose(1, 2).contiguous().view(B, N, (self.n_his*self.time_step+1)*self.state_dim)

        # add offset to center-of-mass for rigids to attr
        # offset: B x N x (n_his * state_dim)
        offset = torch.zeros(B, N, (self.n_his*self.time_step+1)*self.state_dim, device=self.device)
        ...
        is_tool_touching = torch.count_nonzero(Rs_cur[:, :, n_p + args.floor_dim:], dim=(1, 2))
        pred_motion = (is_tool_touching > 0).int().view(-1, 1, 1) * batch_denormalize(motion_norm, mean_d, std_d)
        if args.loss_type == 'chamfer_emd':
            pred_pos = state_cur[:, :n_p, :3] + torch.clamp(pred_motion, max=self.time_step*args.motion_bound, min=-self.time_step*args.motion_bound)
        else:
