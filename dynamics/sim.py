...
class MPM(object):
    ...
    def look_at(self, position):
        ...
        if forward[0] * forward[2] > 0:
            y_rot += np.pi
        x_rot = np.arccos(np.dot(forward, forward_proj) / (np.linalg.norm(forward) * np.linalg.norm(forward_proj)))
        return x_rot, y_rot
    ...
    def normalize_state(self, state_cur_dense):
        ...
        self.scale_factor = self.scale[0] / (np.max(state_cur[:, 0]) - np.min(state_cur[:, 0]))
        self.state_mean = np.mean(state_cur, axis=0)
        state_cur_norm = (state_cur - self.state_mean) * self.scale_factor
        state_cur_norm = np.stack([state_cur_norm[:, 0], state_cur_norm[:, 2], -state_cur_norm[:, 1]]).T
        ...
        for b in range(B):
            ...
            for i in range(act_seqs.shape[1]):
                if 'gripper' in args.env:
                    ...
                    for j in range(len(args.tool_dim[args.env])):
                        ...
                        tool_center_list.append(np.mean(init_pose_seqs[b, i, tool_start:tool_start+tool_dim], axis=0))
                        tool_start += tool_dim
                    ...
                    if 'square' in args.env:
                        ...
                        tool_rot_theta = np.arctan2(plane_vec[1], plane_vec[0]) - np.pi / 4
                        tool_rot = axangle2quat([0, 1, 0], tool_rot_theta)
                    else:
                        tool_rot = [1.0, 0.0, 0.0, 0.0]

                tool_center_norm = (tool_center - np.mean(state_cur, axis=0)) * self.scale_factor
                tool_center_norm = np.stack([tool_center_norm[:, 0], tool_center_norm[:, 2], -tool_center_norm[:, 1]]).T
                ...
                for j in range(len(args.tool_sim_primitive_mapping[args.env])):
                    ...
                    self.env.primitives.primitives[prim_idx].set_state(0, [*tool_center_norm[j], *tool_rot])

                for j in range(act_seqs.shape[2]):
                    action_list = []
                    for k in range(len(args.tool_dim[args.env])):
                        action_list.append(np.array([
                            act_seqs[b, i, j, 6*k], act_seqs[b, i, j, 6*k+2], -act_seqs[b, i, j, 6*k+1], 0.0, 0.0, 0.0,
                        ]) * self.scale_factor / self.action_dim)
                    if 'gripper' in args.env:
                        prim_idx_1, prim_idx_2 = args.tool_sim_primitive_mapping[args.env]
                        action = np.concatenate([np.zeros(6*prim_idx_1), action_list[0], np.zeros(6*(prim_idx_2-prim_idx_1-1)),
                            action_list[1], np.zeros(6*(6-prim_idx_2-1))], axis=0)
                    else:
                        prim_idx = args.tool_sim_primitive_mapping[args.env][0]
                        action = np.concatenate([np.zeros(6*prim_idx), action_list[0], np.zeros(6*(6-prim_idx-1))], axis=0)
                    ...
                    if len(rollout_path) > 0:
                        ...
                        for cam_idx in range(4):
                            cv.imwrite(f"{rollout_path}/{idx:03d}_rgb_{cam_idx+1}.png", rgb[cam_idx][..., ::-1])
            ...
            if len(rollout_path) > 0:
                for cam_idx in range(4):
                    ...
                    subprocess.run(['ffmpeg', '-y', '-i', f'{rollout_path}/%03d_rgb_{cam_idx+1}.png', '-c:v', 'libx264', 
                        '-vf', 'fps=15', '-pix_fmt', 'yuv420p', f'{rollout_path}/{args.env}_sim_vid_{cam_idx+1}_{time_now}.mp4'], 
                        stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
                # os.system(f'rm {rollout_path}/*.png')
                subprocess.run(f'rm {rollout_path}/*.png', shell=True)

        states_pred_array = torch.tensor(np.stack(state_seq_list), dtype=torch.float32, device=args.device)
...
if __name__ == "__main__":
    ...
    state_cur = load_data(args.data_names, 'target_shapes/alphabet_sim/start/start.h5')[0][:args.n_particles]
    ...
    state_seqs = sim.rollout(state_cur, init_pose_seq[None], act_seq[None], rollout_path=rollout_path)
