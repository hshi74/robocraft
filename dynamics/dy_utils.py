...
def compute_stats(args, particles):
    ...
    if args.surface_sample:
        mean_d = torch.mean(particles[:, 1:, :args.n_particles] - particles[:, :-1, :args.n_particles], (1, 2))
        std_d = torch.mean(torch.std(particles[:, 1:, :args.n_particles] - particles[:, :-1, :args.n_particles], 2), 1)
    else:
        d_list = []
...
def find_relations_neighbor(kd_tree, pos, anchor_idx, radius, order, query_idx=None):
    ...
    if query_idx is None:
        ...
        relations[:, 1] = anchor_idx[np.fromiter(itertools.chain.from_iterable(neighbor_ind), dtype=np.int16)]
    else:
        relations[:, 0] = anchor_idx[np.fromiter(itertools.chain.from_iterable(neighbor_ind), dtype=np.int16)]
...
def compute_neighbor_matrix(kd_tree, radius, order, other=None):
    if other is None:
        sps_dist_mat = kd_tree.sparse_distance_matrix(kd_tree, radius, p=order, output_type='coo_matrix')
    else:
        sps_dist_mat = kd_tree.sparse_distance_matrix(other, radius, p=order, output_type='coo_matrix')
...
def prepare_input(args, state_cur, rels_list_prev=None, device='cpu'):
    ...
    for k in range(B):
        ...
        for i in range(len(args.tool_dim[args.env])):
            ...
            sdm = kd_tree.sparse_distance_matrix(tool_kd_tree, args.tool_neighbor_radius[i], output_type='ndarray')
        ...
        if len(rels_list) == 0 and rels_list_prev is not None:
            ...
            p2p_rel = find_relations_neighbor(kd_tree, state, particle_ind, args.neighbor_radius, 2)
