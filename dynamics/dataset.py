...
class GNNDataset(Dataset):
    def __init__(self, args, phase):
        ...
        for gt_vid_path in vid_path_list:
            ...
            for i in range(n_frames):
                gt_frame_data = load_data(args.data_names, os.path.join(gt_vid_path, f'{str(i).zfill(3)}.h5'))[0]
                gt_state = torch.tensor(gt_frame_data, device=args.device, dtype=torch.float32)
            ...
            state_seq_list = []
            for i in range(frame_start, n_frames - args.time_step * args.data_time_step * (args.n_his + args.sequence_length - 1)):
                state_seq = []
                # history frames
                for j in range(i, i + args.time_step * (args.n_his - 1) + 1, args.time_step):
                    # print(f'history: {j}')
    ...
    def __getitem__(self, idx):
        ...
        action_seq = self.action_data_list[idx_vid][idx_curr: idx_curr + args.data_time_step * args.time_step * \
