    ...
    for dataset in ['test', 'valid', 'train']:
        ...
        for idx in tqdm(range(min(args.n_rollout, dataset_size))):
            ...
            for step in range(0, n_frames, args.data_time_step):
                ...
                gt_data = load_data(args.data_names, os.path.join(gt_vid_path, frame_name))[0]
            ...
            if args.data_type == 'synthetic':
                ...
                for step in frame_list:
                    frame_name = str(step).zfill(3) + '.h5'
                    data = load_data(args.data_names, os.path.join(vid_path, frame_name))
            ...
            if args.surface_sample:
                ...
                for i in range(state_pred_seq.shape[0]):
                    ...
                    state_pred_pcd.points = o3d.utility.Vector3dVector(state_pred_upsample)
                    surf_mesh = alpha_shape_mesh_reconstruct(state_pred_pcd, alpha=0.01)
                    state_pred_pcd_surf = o3d.geometry.TriangleMesh.sample_points_poisson_disk(
                ...
                state_pred_surf_seq = np.concatenate((np.array(state_pred_surf_seq), 
                    state_pred_seq[:, args.n_particles:]), axis=1)
            ...
            for i in range(state_seq.shape[0]):
                ...
                if args.data_type == 'synthetic':
                    ...
                    loss_dict['chamfer']['surf'].append(chamfer(state_pred, gt_target_state))
                    loss_dict['emd']['surf'].append(emd(state_pred, gt_target_state))
                    loss_dict['h']['surf'].append(hausdorff(state_pred, gt_target_state))
                else:
            ...
            if args.data_type == 'synthetic':
                render_anim(gnn.args, [f'Eval', 'Synthetic', 'GT'], [state_pred_seq, state_seq, state_gt_seq], 
                    path=os.path.join(dy_anim_path, f'{dataset}_{str(idx).zfill(3)}.mp4'))
            else:
                render_anim(gnn.args, [f'Eval', 'GT'], [state_pred_seq, state_seq], 
                    path=os.path.join(dy_anim_path, f'{dataset}_{str(idx).zfill(3)}.mp4'))
