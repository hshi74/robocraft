...
def train(args, model):
    ...
    for epoch in range(start_epoch, start_epoch + args.n_epoch):
        for phase in phases:
            ...
            for i, (state_seq, action_seq) in enumerate(tqdm(dataloaders[phase], desc=f'Epoch {epoch}/{start_epoch + args.n_epoch}')):
                B = state_seq.size(0)
                n_nodes = state_seq.size(2)

                memory_init = model.init_memory(B, n_nodes)
                groups_gt = get_env_group(args, B)
                floor_state =  state_seq[:, 0, args.n_particles:args.n_particles+args.floor_dim]
                ...
                for j in range(args.sequence_length):
                    with torch.set_grad_enabled(phase=='train'):
                        action_his = action_seq[:, args.n_his*args.time_step*j:args.n_his*args.time_step*(j+1)+1]
                        if j == 0:
                            state_cur = state_seq[:, args.n_his-1, :args.n_particles]
                        else:
                            if args.state_dim == 6:
                                pred_normals = get_normals(pred_pos_p, pkg='torch').to(args.device)
                                state_cur = torch.cat([pred_pos_p, pred_normals], dim=2)
                            else:
                                state_cur = pred_pos_p
                        state_cur = torch.cat([state_cur, floor_state, action_his[:, -1]], dim=1)

                        Rr_curs, Rs_curs, _, _ = prepare_input(args, state_cur[:, :, :3].detach().cpu().numpy(), device=args.device)
                        inputs = [attrs, state_cur, action_his, Rr_curs, Rs_curs, memory_init, groups_gt, stats]
                        ...
                        if args.loss_type == 'chamfer_emd':
                            if args.chamfer_weight > 0:
                                loss += args.chamfer_weight * chamfer_loss(pred_pos_p, gt_pos_p)
