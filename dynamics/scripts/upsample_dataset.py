...
def upsample(state, n_particles, visualize=False):
    ...
    state_upsample_pcd = o3d.geometry.TriangleMesh.sample_points_poisson_disk(state_surf_mesh, n_particles)
    ...
    for phase in ['train', 'valid', 'test']:
        ...
        for i in tqdm(range(len(seq_path_list))):
            ...
            for j in range(len(sample_path_list)):
                ...
                state_path_list = sorted(glob.glob(os.path.join(sample_path_list[j], '*.h5')))
                if len(state_path_list) < 3: 
                    continue

                new_sample_path = sample_path_list[j].replace(src_dir, dest_dir)
                if os.path.exists(new_sample_path) and len(glob.glob(os.path.join(new_sample_path, '*'))) > 3:
                    continue
                ...
                for k in range(len(state_path_list)):
                    ...
                    store_data(data_names, new_state_data, os.path.join(new_sample_path, f'{str(k).zfill(3)}.h5'))

                shutil.copy(os.path.join(sample_path_list[j], 'param_seq.npy'), new_sample_path)
                if os.path.exists(os.path.join(sample_path_list[j], 'data.png')):
                    shutil.copy(os.path.join(sample_path_list[j], 'data.png'), new_sample_path)


def main():
    src_dir = 'data/planning/data_gripper_sym_rod_robot_v4_surf_nocorr_full_normal_keyframe=16_action=2_p=300'
    n_particles = 1024
