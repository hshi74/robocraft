...
def ball_pivoting_mesh_reconstruct(pcd, radii = [0.001, 0.002, 0.004, 0.008], visualize=False):
...
def vg_filter(cube, sampled_points, grid_size=0.001, axis=2, visualize=False):
    ...
    exists_mask = np.array(cube_grid.check_if_included(o3d.utility.Vector3dVector(test_points.reshape((-1, 3)))))
    exists_mask = exists_mask.reshape((-1, ax_grid.shape[0]))
    if axis == 2:
        vg_up_mask = np.sum((tiled_points[:, :, axis] < ax_grid) * exists_mask, axis=1) > 0
        vg_down_mask = np.sum((tiled_points[:, :, axis] > ax_grid) * exists_mask, axis=1) > 0
...
def o3d_registration(pcds_down, max_correspondence_distance_fine=0.002, 
    visualize=False):
...
def scalable_integrate_rgb_frames(args, rgbd_images, visualize=False):
    ...
    depth_T = np.concatenate((np.concatenate((quat2mat(args.depth_optical_frame_pose[3:]), 
        np.array([args.depth_optical_frame_pose[:3]]).T), axis=1), [[0, 0, 0, 1]]))

    for i in range(len(pose_graph_rgbd.nodes)):
        intrinsic = o3d.io.read_pinhole_camera_intrinsic(
            f"perception/config/intrinsics_{i+1}.json")
        cam_T = np.concatenate((np.concatenate((quat2mat(args.cam_pose_dict[f"cam_{i+1}"]["orientation"]), 
            np.array([args.cam_pose_dict[f"cam_{i+1}"]["position"]]).T), axis=1), [[0, 0, 0, 1]]))
...
if __name__ == "__main__":
    ...
    random_quat = torch.tensor(np.random.randn(4,))
