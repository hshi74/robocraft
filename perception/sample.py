...
def merge_point_cloud(args, pcd_msgs, crop_range=None, visualize=False):
    ...
    for i in range(len(pcd_msgs)):
        ...
        points = (quat2mat(args.depth_optical_frame_pose[3:]) @ points.T).T + args.depth_optical_frame_pose[:3]
        cam_ori = args.cam_pose_dict[f"cam_{i+1}"]["orientation"]
        points = (quat2mat(cam_ori) @ points.T).T + args.cam_pose_dict[f"cam_{i+1}"]["position"]
        ...
        if crop_range is None:
            ...
            z_filter = (points.T[2] > args.mid_point[2] + crop_range[2]) & (points.T[2] < args.mid_point[2] + crop_range[5])
...
def inside_tool_filter(sampled_points, tool_list, in_d=0.0, close_d=-0.01, visualize=False):
    ...
    if visualize:
        ...
        visualize_o3d([*list(zip(*tool_list))[1], out_tool_pcd, in_tool_pcd], title='inside_tool_filter')
...
# @profile
def sample(args, pcd, pcd_dense_prev, pcd_sparse_prev, tool_list, is_moving_back, patch=False, visualize=False):
    ...
    if args.surface_sample:
        ...
        if visualize:
            visualize_o3d([fps_pcd, *list(zip(*tool_list))[1]], title='fps_point_cloud', pcd_color=color_avg)
...
# @profile
def ros_bag_to_pcd(args, bag_path, tool_list, pcd_dense_prev=None, pcd_sparse_prev=None, image_path_prefix='', 
    last_dist=float('inf'), is_moving_back=False, visualize=False, write=False):
    pcd_msgs = []
    try:
...
def main():
    ...
    for i in range(int(start_idx), int(start_idx) + int(n_vids)): # len(dir_list)
        ...
        for j in tqdm(range(start_frame, len(bag_list), step_size), desc=f'Video {vid_idx} {params}'): # len(bag_list)
            ...
            pcd_dense, pcd_sparse, state_cur, last_dist, is_moving_back = ros_bag_to_pcd(
                args, bag_list[j], tool_list, pcd_dense_prev=pcd_dense_prev, pcd_sparse_prev=pcd_sparse_prev, 
            ...
            store_data(args.data_names, h5_data, os.path.join(rollout_path, str(j).zfill(3) + '.h5'))

            # state_prev = state_cur

        render_anim(args, ['Perception'], [np.array(state_seq)], path=os.path.join(rollout_path, 'repr.mp4'))
        if write_frames:
            render_frames(args, ['Perception'], [np.array(state_seq)], views=[(90, 0)], path=os.path.join(rollout_path, 'frames'))
        if write_gt_state:
            os.system(f'ffmpeg -f image2 -framerate 20 -start_number {start_frame} -i {image_path}/%03d_raw.png {rollout_path}/raw.mp4')
