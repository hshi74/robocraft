...
class Capsule(Primitive):
    def __init__(self, **kwargs):
        ...
        self.h = ti.field(self.dtype, shape=())  # self.cfg.h
        self.r = ti.field(self.dtype, shape=())  # self.cfg.r
        # self.h = self.cfg.h
        # self.r = self.cfg.r
    ...
    def initialize(self):
        ...
        self.r[None] = self.cfg.r

    @ti.func
    def _sdf(self, f, grid_pos):
        p2 = grid_pos
        p2[1] += self.h[None] / 2
        p2[1] -= min(max(p2[1], 0.0), self.h[None])
        return length(p2) - self.r[None]
...
class Box(Primitive):
    ...
    def _sdf(self, f, grid_pos):
        # p: vec3,b: vec3
        # cosine * samplePosition.x + sine * samplePosition.y, cosine * samplePosition.y - sine * samplePosition.x
        # angle = np.pi / 2
        # c = ti.cos(angle)
        # s = ti.sin(angle)
        # grid_pos[0] = c * grid_pos[0] - s * grid_pos[2]
        # grid_pos[2] = s * grid_pos[0] + c * grid_pos[2]
...
class Gripper(Capsule):
    ...
    def _sdf(self, f, grid_pos):
        delta = ti.Vector([self.gap[f] / 2, 0., 0.])
        p = grid_pos - ti.Vector([0., -self.h / 2, 0.])
        a = super(Gripper, self)._sdf(f, p - delta)  # grid_pos - (mid + delta)
        b = super(Gripper, self)._sdf(f, p + delta)  # grid_pos - (mid - delta)
        return ti.min(a, b)
    ...
    def _normal(self, f, grid_pos):
        ...
        a = super(Gripper, self)._sdf(f, p - delta)  # grid_pos - (mid + delta)
        b = super(Gripper, self)._sdf(f, p + delta)  # grid_pos - (mid - delta)
        a_n = super(Gripper, self)._normal(f, p - delta)  # grid_pos - (mid + delta)
        b_n = super(Gripper, self)._normal(f, p + delta)  # grid_pos - (mid + delta)
        m = ti.cast(a <= b, self.dtype)
        return m * a_n + (1 - m) * b_n
...
#         self.r = self.cfg.r
#         self.minimal_gap = self.cfg.minimal_gap
#         assert self.action_dim == 4 # 3 linear, 1 for grasp ..
#
#     @ti.kernel
#     def forward_kinematics(self, f: ti.i32):
#         self.gap[f+1] = max(self.gap[f] - self.gap_vel[f], self.minimal_gap)
...
#             # for k in ti.static(range(3)):
#             #     self.w[j][k] = self.action_buffer[s][k+3] * self.action_scale[None][k+3]/n_substeps
#             self.gap_vel[j] = self.action_buffer[s][3] * self.action_scale[None][3]/n_substeps
#
#     @ti.func
#     def _sdf(self, f, grid_pos):
#         # delta = ti.Vector([self.gap[f] / 2, 0., 0.])
#         # p = grid_pos - ti.Vector([0., -self.size[None][1]/2, 0.])
#         # a = super(Gripper, self)._sdf(f, p-delta) # grid_pos - (mid + delta)
#         # b = super(Gripper, self)._sdf(f, p+delta) # grid_pos - (mid - delta)
#         # return ti.min(a, b)
#         delta = ti.Vector([self.gap[f] / 2, 0., 0.])
...
#         self.size[None] = self.cfg.size
#         self.minimal_gap = self.cfg.minimal_gap
#         assert self.action_dim == 4 # 3 linear, 1 grasp ..
#
#     @ti.kernel
#     def forward_kinematics(self, f: ti.i32):
#         self.gap[f+1] = max(self.gap[f] - self.gap_vel[f], self.minimal_gap)
...
#     def _sdf(self, f, grid_pos):
#         delta = ti.Vector([self.gap[f] / 2, 0., 0.])
#         p = grid_pos - ti.Vector([0., -self.size[1]/2, 0.])
#         a = super(Gripper, self)._sdf(f, p-delta) # grid_pos - (mid + delta)
#         b = super(Gripper, self)._sdf(f, p+delta) # grid_pos - (mid - delta)
#         return ti.min(a, b)
#
