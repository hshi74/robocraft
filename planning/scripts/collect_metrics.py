...
def main():
    ...
    for letter in letter_names:
        dump_dir_list_glob = sorted(glob.glob(os.path.join(control_dir, test_type, letter, '*')))
        for dump_dir in dump_dir_list_glob:
            # import pdb; pdb.set_trace()
            if os.path.exists(f"{dump_dir}/anim_args/MPC_anim_args.pkl") and \
                keyword in dump_dir and not 'debug' in dump_dir: # date in dump_dir and 
    ...
    for dump_dir in dump_dir_list:
        ...
        if not os.path.exists(res_anim):
            os.system(f"python utils/visualize.py {dump_dir}/anim_args/MPC_anim_args.pkl")
        
        shutil.copyfile(res_anim, f'{inspect_dir}/anim/{target_name}_{test_name}.mp4')
        shutil.copyfile(f'{dump_dir}/param_seqs/MPC_param_seq.yml', f'{inspect_dir}/soln/{target_name}_{test_name}.yml')
