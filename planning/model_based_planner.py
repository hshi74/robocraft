...
class ModelBasedPlanner(object):
    def __init__(self, args, plan_params, is_3d=False, model_path=None):
        ...
        self.act_len = int(plan_params['act_len'] / args.time_step)
    ...
    def plan(self, state_cur, target_shape, rollout_path, max_n_actions, rs_loss_threshold=float('inf')):
        ...
        self.param_bounds = get_param_bounds(self.plan_params, min_bounds, max_bounds)
        ...
        if self.args.optim_algo == 'RS':
            ...
            with torch.set_grad_enabled(True):
                param_seq_opt = self.optimize_GD(param_seq_pool, state_cur, target_shape)
            ...
            param_seq_CEM, loss_seq_CEM, = self.optimize_CEM(
                (param_seq_pool, loss_seq_pool, state_seq_pool), state_cur, target_shape)
            with torch.set_grad_enabled(True):
                param_seq_opt = self.optimize_GD(param_seq_pool, state_cur, target_shape, guide=param_seq_CEM)
    ...
    def rollout(self, param_seqs, state_cur, target_shape):
        ...
        #     init_pose_seq = params_to_init_pose(self.args, self.center, self.plan_params, param_seq.cpu())
        #     act_seq = params_to_actions(self.plan_params, param_seq.cpu())
        ...
        # print(torch.where(act_seqs_backup - act_seqs.cpu() > 0.0001))
    ...
    def eval_soln(self, param_seq, state_cur, target_shape):
        ...
        with open(pkl_path, 'wb') as f:
            pickle.dump({'args': self.args, 'row_titles': [title], 'state_seqs': [state_seq_wshape], 'target': target_shape['surf']}, f)
    ...
    def random_search(
        ...
        loss_seq_list, state_seq_list = self.rollout(param_seqs, state_cur, target_shape)
        ...
        for i in range(max_n_actions):
            ...
            state_seq_opt.extend([state_seq_list[x][:(i+1)*self.act_len] for x in best_loss_ind[:, i]])
        ...
        self.render_selections('RS', state_cur, param_seq_opt, loss_opt, state_seq_opt, target_shape)
    ...
    def optimize_CEM(    
        ...
        for i, (param_seq_cur, loss_cur, state_seq_cur) in enumerate(zip(*soln_pool)):
            ...
            for j in range(self.CEM_sample_iter):
                ...
                loss_list, state_seq_list = self.rollout(param_seq_CEM, state_cur, target_shape)
    ...
    def render_selections(self, name, state_cur, param_seq_opt, loss_opt, state_seq_opt, target_shape):
        ...
        with open(pkl_path, 'wb') as f:
            pickle.dump({'args': self.args, 'row_titles': row_titles, 'state_seqs': state_seq_wshape_list, 
                'target': target_shape['surf']}, f)
    ...
    def render_state(self, state_cur, target_shape, state_pred=None, pred_err=0, frame_suffix='model'):
        ...
        if frame_suffix == 'before' and state_pred is not None:
            ...
            render_frames(self.args, [f'State', f'State Pred={round(pred_err.item(), 6)}', 'State Normalized', 'State Pred Normalized'], 
                [state_cur.cpu()[-1:], state_pred.cpu()[-1:], state_cur_norm.cpu()[-1:], state_pred_norm.cpu()[-1:]], 
            ...
            render_frames(self.args, [f'State', 'State Normalized'], 
                [state_cur.cpu()[-1:], state_cur_norm.cpu()[-1:]], axis_off=False, focus=[True, True], 
                target=[state_goal.cpu()[0], state_goal_norm.cpu()[0]], path=os.path.join(self.rollout_path, 'states'), 
                name=f"{self.args.env}_state_{frame_suffix}.png")
    ...
    def optimize_GD(
        ...
        for i in range(len(param_seq_pool)):
            ...
            def loss_func(param_seq):
                ...
                loss, state_seq = self.rollout(param_seq.unsqueeze(0), state_cur, target_shape)
